...
export default function TradeHistory({ trades, minConfidence = 0.6 }) {
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {filteredTrades.map((trade, idx) => {
              const tradeTime = trade.timestamp ? new Date(trade.timestamp * 1000).toLocaleTimeString() : '—'
              ...
              return (
                <React.Fragment key={`${trade.timestamp || 't'}-${idx}`}>
                  <tr
                  ...
                  >
                    ...
                    <td className="py-2 px-2 text-right font-mono text-xs">
                      ${toNum(trade.price).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </td>

                    <td className="py-2 px-2 text-right font-mono text-xs">
                      {(toNum(trade.confidence) * 100).toFixed(0)}%
                    </td>
