...
def test_pycryptodome() -> bool:
    ...
    try:
        ...
        print_fail("pycryptodome NOT INSTALLED → pip install pycryptodome")
        return False
    except Exception as e:
...
def test_dependencies() -> bool:
    ...
    for module_name, pip_name in required:
        try:
            ...
            print_fail(f"{pip_name} → pip install {pip_name}")
...
def test_env_vars() -> bool:
    ...
    if private_key:
        ...
        ok = False
    if openrouter_key:
        print_ok(f"OPENROUTER: sk-or-...OK")
    else:
        print_warn("OPENROUTER_API_KEY → LLM fallback hold")
    if dashboard_key:
        print_ok(f"DASHBOARD_API_KEY: {len(dashboard_key)} chars ✓")
    else:
        print_warn("DASHBOARD_API_KEY → auto-generated by api_server.py")
    print_info(f"MODE: {mode} | MAINNET: {mainnet}")
    if mode == "live" and mainnet.lower() == "true":
        print_warn("⚡ LIVE + MAINNET=TRUE → SOLDI VERTI!")
    return ok
...
def test_api_server() -> bool:
    ...
    if not api_key:
        print_warn("No DASHBOARD_API_KEY → skip (set in .env)")
        return True
    import requests
    ...
    for ep in endpoints:
        try:
            resp = requests.get(f"{base_url}{ep}", headers={"X-API-Key": api_key}, timeout=5)
            if resp.status_code == 200:
                print_ok(f"{ep}: OK")
            else:
                print_fail(f"{ep}: {resp.status_code}")
                ok = False
        except Exception as e:
            print_fail(f"{ep}: {e}")
            ok = False
...
def test_frontend_deps() -> bool:
    print_header("9. FRONTEND")
    frontend_dir = Path("frontend")
    if not frontend_dir.exists():
        print_fail("frontend/ missing → cd frontend && npm install")
        return False
    ok, out = run_command(["npm", "list"], cwd="frontend")
    if ok:
        print_ok("npm deps OK")
    else:
        print_warn("npm deps issue → cd frontend && npm install")
    ok2, _ = run_command(["npm", "run", "dev", "-- --port=3000"], cwd="frontend", timeout=3)
...
def test_readiness() -> bool:
    ...
    if score >= 90:
        print_success("🚀 BOT READY — Run: python hyperliquid_bot_executable_orders.py --single-cycle")
    elif score >= 70:
